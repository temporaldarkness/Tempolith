...
public sealed partial class BanListEui : BaseEui
{
    ...
    private void OnLineIdsClicked<T>(IBanListLine<T> line) where T : SharedBan
    {
        ...
        var ipArray = new string[ban.Addresses.Length];
        for (var i = 0; i < ban.Addresses.Length; i++)
        {
            ipArray[i] = ban.Addresses[i].address;
        }

        var ip = ban.Addresses.Length == 0
            ? string.Empty
            : Loc.GetString("ban-list-ip", ("ip", string.Join(',', ipArray)));
