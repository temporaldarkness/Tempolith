...
public partial class InteractionsPanel
{
    ...
    private void OnInteractionMessageReceived(Entity<InteractionsComponent> ent, ref InteractionMessage args)
    {
        ...
        var userPref = true; //_playerCacheManager.GetEmoteVisibility(userSession.UserId);
        var targetPref = true; //_playerCacheManager.GetEmoteVisibility(targetIsPlayer ? targetSession!.UserId : null);
        ...
        {
            _chatSystem.SendInVoiceRange(ChatChannel.Emotes, MetaData(ent.Owner).EntityName, rawMsg, msg, obfuscated: "", obfuscatedWrappedMessage: "", ent.Owner, ChatTransmitRange.Normal, color: Color.Pink);
        }
        else
        {
        ...
        bool targetIsPlayer)
    {
        ...
        if (userPref && targetPref && targetIsPlayer)
        {
            _chatSystem.SendInVoiceRange(ChatChannel.Emotes, MetaData(user).EntityName, data.InteractionMessage, msg, obfuscated: "", obfuscatedWrappedMessage: "", user, ChatTransmitRange.Normal, color: Color.Pink);
        }
        else
        {
            var filter = Filter.Empty();
